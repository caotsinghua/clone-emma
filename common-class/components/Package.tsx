...
class Package extends PureComponent<WithSearchContext<WithStdin<Props>>, State> {
  state: State = {
    showDetails: false
  };
   ...
   * @param state
   */
  static getDerivedStateFromProps(props: WithSearchContext<WithStdin<Props>>, state: State): State | null {
  ...
  render() {
    ...
    const Downloads = () => (
      <Box width={this.getColumnWidth('humanDownloadsLast30Days')} marginRight={1}>
      ...
      <SearchContext.Consumer>
        {values => (
          <StdinContext.Consumer>
            {({ stdin, setRawMode }) => <Package {...this.props} stdin={stdin} setRawMode={setRawMode} hits={values} />}
          </StdinContext.Consumer>
        )}
      </SearchContext.Consumer>
    );
