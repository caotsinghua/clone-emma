    ...
    return (
      <SearchContext.Provider value={hits}>
        <Box flexDirection="column">
          <Search value={query} onChange={this.handleQueryChange} loading={loading} active />
          <Scroll values={this.state.hits} onWillReachEnd={this.handleWillReachEnd} active={view === 'SCROLL'}>
            {pkg => (
              <Package
                key={pkg.objectID}
                pkg={pkg}
  ...
  render() {
    return (
      <StdinContext.Consumer>
        {({ stdin, setRawMode }) => <Emma {...this.props} stdin={stdin} setRawMode={setRawMode} />}
      </StdinContext.Consumer>
    );
  }
}
